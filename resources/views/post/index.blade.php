...
<div class="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <!-- Category Filter -->
        <div class="mb-8">
            <form method="GET" action="{{ route('posts.index') }}" class="max-w-md mx-auto">
                <div class="flex items-center space-x-4">
                    <label for="category" class="flex items-center text-gray-700 font-medium">
                    ...
                    <select name="category" id="category" onchange="this.form.submit()" 
                            class="flex-1 px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 bg-white">
                        <option value="">🍽️ Todas las recetas</option>
                        <option value="comida" {{ $selectedCategory == 'comida' ? 'selected' : '' }}>🥘 Comidas</option>
                        <option value="postre" {{ $selectedCategory == 'postre' ? 'selected' : '' }}>🍰 Postres</option>
                    </select>
                </div>
            </form>
        </div>
        ...
        @if($posts->count() > 0)
            ...
            <div class="mb-8 text-center">
                <p class="text-gray-600">
                    ...
                    {{ $posts->count() == 1 ? 'receta tradicional' : 'recetas tradicionales' }} 
                    @if($selectedCategory)
                        de <span class="font-semibold text-orange-600">{{ $selectedCategory == 'comida' ? 'comidas' : 'postres' }}</span>
                    @endif
                    para descubrir
                </p>
            </div>
                ...
                @foreach($posts as $post)
                    <article class="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                        ...
                        <div class="p-6">
                            <!-- Category Badge -->
                            <div class="mb-3">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $post->category == 'comida' ? 'bg-orange-100 text-orange-800' : 'bg-pink-100 text-pink-800' }}">
                                    {{ $post->category == 'comida' ? '🥘' : '🍰' }}
